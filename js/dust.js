...
function init() {
  ...
  function handleSuccess(stream) {
    ...
    dataGeo = new THREE.SphereGeometry(0.5, 20, 32);
    ...
    for (let i = 0; i < 500; i++) {
      ...
      const s = i / 2;
      scene.add(dataTube);
      dataTube.position.set(Math.random(s), Math.random(s), Math.sin(s));
...
function render() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    acceleration = new THREE.Vector3(Math.sin(v), Math.sin(newMap), v);
